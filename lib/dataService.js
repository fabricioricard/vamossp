...
class DataService {
  ...
  async scrapeNews(existingInstitucional = []) {
    // Função para verificar se a notícia tem imagem e é relevante
    const isValidNews = (title, image, content = '') => {
      // Verifica se tem imagem
      if (!image || image.trim() === '') {
        console.log(`[DataService] ❌ Ignorando sem imagem: ${title.substring(0, 50)}...`);
        return false;
      }
      
      // Verifica se é sobre São Paulo (não é Copa do Mundo, seleção, etc)
      const text = (title + ' ' + content).toLowerCase();
      const irrelevantKeywords = ['copa do mundo', 'seleção brasileira', 'seleção', 'tite', 'dorival júnior seleção', 'neyymar seleção', 'copa américa', 'eliminatórias'];
      const isIrrelevant = irrelevantKeywords.some(keyword => text.includes(keyword));
      
      if (isIrrelevant) {
        console.log(`[DataService] ❌ Ignorando notícia irrelevante: ${title.substring(0, 50)}...`);
        return false;
      }
    ...
    try {
      ...
      for (const link of newsLinks) {
        ...
        try {
          ...
          // Extrai conteúdo para validação
          ...
          $article('article p, .mc-article-body p, .content-text__container p, .matter__content p').each((i, p) => {
            ...
            if (text && text.length > 50 && !text.includes('Veja também') && !text.includes('Leia mais')) {
              contentForValidation += text + ' ';
            }
          });
          
          // Valida se a notícia tem imagem e é relevante
          if (!isValidNews(title, image, contentForValidation)) {
      ...
      try {
        ...
        for (const link of newsLinks) {
          ...
          try {
            ...
            // Valida se a notícia tem imagem e é relevante
            if (!isValidNews(title, image, contentSection)) {
              await new Promise(resolve => setTimeout(resolve, 300));
              continue;
            }
      ...
      if (firstLink) {
        ...
        if (!alreadyExists) {
          try {
            ...
            // Extrair conteúdo para validação
            const contentMatchValidation = html.match(/<article[^>]*>([\s\S]*?)<\/article>/i);
            let contentForValidation = '';
            if (contentMatchValidation) {
              const pRegexValidation = /<p[^>]*>([\s\S]*?)<\/p>/gi;
              let pMatchValidation;
              while ((pMatchValidation = pRegexValidation.exec(contentMatchValidation[1])) !== null) {
                let text = pMatchValidation[1].replace(/<[^>]+>/g, '').trim();
                if (text.length > 30) {
                  contentForValidation += text + ' ';
                }
              }
            ...
            // Valida se a notícia tem imagem e é relevante
            if (!isValidNews(firstLink.title, image, contentForValidation)) {
              console.log(`[DataService] SPFC.NET: Notícia sem imagem ou irrelevante, pulando`);
            } else {
